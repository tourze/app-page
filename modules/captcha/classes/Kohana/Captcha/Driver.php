 ...
 * @author		YwiSax
 */
abstract class Kohana_Captcha_Driver {

	const DEFAULT_DRIVER_PREFIX = 'Captcha_Driver_';
	...
	/**
	 * @var  string  图片类型("png", "gif" or "jpeg")
	 */
	protected $image_type = 'png';

	public static function factory($config, $group = NULL)
	{
		// 设置类名
		$class = Captcha_Driver::DEFAULT_DRIVER_PREFIX . ucfirst($config['style']);
	...
	public function __construct($group = NULL)
	{
		...
		{
			throw new Captcha_Exception('Captcha group not defined in :group configuration', array(
				':group' => $group
			));
		}
		...
		if ($group !== 'default')
		{
			...
			if ( ! is_array($default = Kohana::$config->load('captcha')->get(Captach::DEFAULT_GROUP)))
			{
				throw new Captcha_Exception('Captcha group not defined in :group configuration', array(
					':group' => 'default'
				));
		...
		{
			if (array_key_exists($key, Captcha_Driver::$config))
			{
				Captcha_Driver::$config[$key] = $value;
			}
		}

		// Store the config group name as well, so the drivers can access it
		Captcha_Driver::$config['group'] = $group;

		// If using a background image, check if it exists
		if ( ! empty($config['background']))
		{
			Captcha_Driver::$config['background'] = str_replace('\\', '/', realpath($config['background']));
			if ( ! is_file(Captcha_Driver::$config['background']))
			{
				throw new Captcha_Exception('The specified file, :file, was not found.', array(
					':file' => Captcha_Driver::$config['background']
				));
			}
		}
		...
		{
			Captcha_Driver::$config['fontpath'] = str_replace('\\', '/', realpath($config['fontpath'])).'/';

			foreach ($config['fonts'] AS $font)
			{
				if ( ! is_file(Captcha_Driver::$config['fontpath'].$font))
				{
					throw new Captcha_Exception('The specified file, :file, was not found.', array(
						':file' => Captcha_Driver::$config['fontpath'].$font
					));
				}
			}
		}
	...
	public function update_response_session()
	{
		...
		//echo "save new captcha {$this->response}<br/>";
		////exit;
		//Session::instance()->set('captcha_response', strtoupper($this->response));
	}
	...
	public function promoted($threshold = NULL)
	{
		...
		if (Captcha_Driver::$config['promote'] === FALSE)
			return FALSE;

		// Use the config threshold
		if ($threshold === NULL)
		{
			$threshold = Captcha_Driver::$config['promote'];
		}
	...
	public function image_create($background = NULL)
	{
		...
		{
			throw new Captcha_Exception('Captcha requires GD2');
		}

		// Create a new image (black)
		$this->image = imagecreatetruecolor(Captcha_Driver::$config['width'], Captcha_Driver::$config['height']);
		...
		if ( ! empty($background))
		{
			...
			if (imagesx($this->background_image) !== Captcha_Driver::$config['width']
			    or imagesy($this->background_image) !== Captcha_Driver::$config['height'])
			{
				imagecopyresampled
				(
					$this->image, $this->background_image, 0, 0, 0, 0,
					Captcha_Driver::$config['width'], Captcha_Driver::$config['height'],
	...
	public function image_gradient($color1, $color2, $direction = NULL)
	{
		...
		$steps = ($direction === 'horizontal') ? Captcha_Driver::$config['width'] : Captcha_Driver::$config['height'];
		...
		if ($direction === 'horizontal')
		{
			...
			$y1 = 0;
			$x2 =& $i;
			$y2 = Captcha_Driver::$config['height'];
		}
		else
		{
			$x1 = 0;
			$y1 =& $i;
			$x2 = Captcha_Driver::$config['width'];
			$y2 =& $i;
		}
	...
	public function image_render($html)
	{
		...
		if ($html === TRUE)
		{
			//$url = URL::site('captcha/' . Captcha_Driver::$config['group']) . '?code=' . Text::random('alnum', 32);
			$url = URL::site('captcha/' . Captcha_Driver::$config['group']);
			return '<img src="'.$url.'" width="'.Captcha_Driver::$config['width'].'" height="'.Captcha_Driver::$config['height'].'" style="cursor:pointer;" onclick="this.src=this.src+\'?\'" alt="' . __('Click to refresh') . '" class="captcha" />';
		}
