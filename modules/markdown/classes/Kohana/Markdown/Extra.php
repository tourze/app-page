 ...
 */
class Kohana_Markdown_Extra extends Markdown {
	...
	const DO_FOOTNOTES_REGEX = '{\[\^(.+?)\]}';
	const DO_FOOTNOTES_REPLACE = "F\x1Afn:\\1\x1A:";
	...
	public function doFootnotes($text)
	{
		if ( ! $this->in_anchor)
		{
			$text = preg_replace(Markdown_Extra::DO_FOOTNOTES_REGEX, Markdown_Extra::DO_FOOTNOTES_REPLACE, $text);
		}
		return $text;
	}
	
	const APPEND_FOOTNOTES_REGEX = '{F\x1Afn:(.*?)\x1A:}';
	...
	{
		$text = preg_replace_callback(Markdown_Extra::APPEND_FOOTNOTES_REGEX, array(&$this, '_appendFootnotes_callback'), $text);
