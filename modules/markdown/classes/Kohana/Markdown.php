 ...
 * Markdown解析器类
 *
 * @package		Kohana/Markdown
 */
abstract class Kohana_Markdown {
	...
	{
		$text = preg_replace_callback(Markdown::BLOCK_QUOTES_REGEX, array(&$this, '_do_block_quotes_callback'), $text);
		return $text;
	}

	/**
	 * [Markdown::do_block_quotes]的回调函数
	 */
	protected function _do_block_quotes_callback($matches)
	{
		...
		$bq = preg_replace_callback('{(\s*<pre>.+?</pre>)}sx', array(&$this, '_do_block_quotes_callback2'), $bq);
	...
	{
		$text = $this->no_entities
			? str_replace('&', '&amp;', $text)
			: preg_replace('/&(?!#?[xX]?(?:[0-9a-fA-F]+|\w+);)/', 
								'&amp;', $text);	// Ampersand-encoding based entirely on Nat Irons's Amputator
	...
	{
		$text = preg_replace_callback(Markdown::DETAB_CALLBACK_REGEX, array(&$this, '_detab_callback'), $text);
		return $text;
	}
	...
	protected function _initDetab()
	{
		...
		// 挺害怕下面的create_function有问题
		$this->utf8_strlen = create_function('$text', 'return preg_match_all(
			"/[\\\\x00-\\\\xBF]|[\\\\xC0-\\\\xFF][\\\\x80-\\\\xBF]*/", 
			$text, $m);');
	}

	const UNHASH_CALLBACK_REGEX = '/(.)\x1A[0-9]+\1/';
	...
	{
		return preg_replace_callback(Markdown::UNHASH_CALLBACK_REGEX, array(&$this, '_unhash_callback'), $text);
	}
