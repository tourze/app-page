...
<?php foreach ($config['name'] AS $i => $_name): ?>
	<?php if ($_name): ?>
	<label class="radio">
		<input type="radio" name="<?php echo $name ?>" value="<?php echo $config['value'][$i] ?>" <?php if ($config['default'][$i]) { ?>checked<?php } ?> />
		<?php echo $_name ?>
	</label>
	<?php endif; ?>
<?php endforeach; ?>
<?php else: ?>
<table class="table admin-cms-field-radio-table">
	<thead>
		<tr>
	...
	<tfoot>
		...
		<tr class="template">
			<td><input type="text" class="name-input" name="config[5][name][]" /></td>
			<td><input type="text" class="value-input" name="config[5][value][]" /></td>
			<td>
				<button class="btn btn-mini btn-block span6 default"><?php echo __('Default') ?></button>
				<button class="btn btn-mini btn-danger btn-block span6 delete"><?php echo __('Delete') ?></button>
				<input type="hidden" class="default-input" name="config[5][default][]" value="0" />
			</td>
		</tr>
	</tfoot>
</table>
