...
$(function(){
	...
	$('#mail-modal .delete').live('click', function(){
		...
		$.post(delete_callback, { C_id:$('#mail-modal-personId').val() }, function(){
			...
			var mail_id = $('#mail-modal-id').val();
			console.log('DELETE MAIL : '+mail_id);
			$('#mail-'+mail_id).fadeOut().remove();
		})
		return false;
	});
	...
	// 提示附件列表
	$('.annex-list').live('click', function(){
		var callback = $('#gduf-mailbox').data('annex-list-callback');
		var current = $(this);
		...
		$.post(callback, {
			...
			for (var i in data)
			{
				content += no+'、<a target="_blank" href="'+data[i].file+'">'+data[i].name+' '+data[i].size+'</a><br />';
				no++;
			}
			current.popover({
				html:		true,
