 ...
 * @author     YwiSax
 */
class Model_Gduf_Mail extends ORM {
	...
	public static function parse_mail_list($response_body)
	{
		...
		if (preg_match_all("/<td width=\"60\"  align=\"left\" valign=\"top\" >(.*)<\/td>/", $response_body, $matches))
		{
			...
			foreach ($downlinks AS $key => $value)
			{
				...
				// 多个附件的时候，会需要打开额外的窗口
				if (preg_match('@javascript:winAnnexOpen\((\d+)\);@', $value, $match))
				{
					//echo Debug::vars($match);
					$value = '<a href="javascript:;" class="annex-list" data-id="'.$match[1].'" data-loaded="0">下载</a>';
				}
	 ...
	 * 获取邮件列表
	 */
	public static function fetch_annex_list($mail_id)
	{
		// http://www.gduf.edu.cn/mail/annex_list.jsp?id=5041046
		$uri = 'mail/annex_list.jsp';
		$params = array(
			'id'	=> $mail_id,
		);
		$response = Model_Gduf_Mail::query_data($uri, $params);
		...
		preg_match_all('@<td align="center" valign="top"><a href=\'../(.*)\' >(.*)&nbsp;</a></td>\r\n\s\s\s\s\s\s<td width="150" align="center" valign="top" >(.*)&nbsp;</td>@', $content, $matches);
