...
/**
 * Modified Preorder Tree Traversal Class. A port of Banks' Sprig_MPTT plus some code from BIakaVeron's ORM_MPTT module.
 *
 * @package		Kohana/ORM
 * @category	Advanced
 */
class Kohana_ORM_MPTT extends ORM {
	...
	public function parents($root = TRUE, $with_self = FALSE, $direction = 'ASC', $direct_parent_only = FALSE)
	{
		...
		return $direct_parent_only ? $query->find() : $query->find_all();
	}
	...
	public function fulltree($scope = NULL)
	{
		...
		{
			$result
				->order_by($this->scope_column, 'ASC')
				->order_by($this->left_column, 'ASC')
				;
		}
	...
	public function descendants($self = FALSE, $direction = 'ASC', $direct_children_only = FALSE, $leaves_only = FALSE, $limit = FALSE)
	{
		...
		{
			$query
				->where($this->right_column, '=', DB::expr($this->left_column.' + 1'))
				;
		}
	...
	public function render_descendants($style = NULL, $self = FALSE, $direction = 'ASC')
	{
		...
		return View::factory($style, array(
			'nodes' => $nodes,
