...
class Kohana_Weibo {
	...
	public static function callback()
	{
		...
		if ($token)
		{
			...
			if ($token['uid'] == Weibo::client('uid')->val)
			{
				...
				// 发布端授权后，跳转到管理后台。
				HTTP::redirect( Route::url('weibo-admin', array('controller' => 'Weibo', 'action' => 'setting')) );
			}
	...
	public static function authentication()
	{
		...
		if ($access_token = Session::instance()->get('access_token'))
		{
			Weibo::$user = Model::factory('Weibo.User')
				->where('access_token', DB_EQUAL, $access_token)
				->find();
	...
	public static function send($status, $img_id = NULL, $token = NULL)
	{
		...
		//echo "poster: " . Weibo::$user->uid . "\n";
		//echo "ip: ". Request::$client_ip . "\n";
		//echo "name: ". Weibo::$user->screen_name . "\n";
		//exit;
		...
		if ($img->loaded())
		{
			...
			$tmp_result = $result = $c->upload($status, DOCROOT.'media'. DIRECTORY_SEPARATOR . $img->filepath);
		}
		else
		{
		...
		else
		{
			...
			unset($tmp_result['id']);
			$tmp_result['user'] = Weibo::$user->as_array();
			// 保存到数据库
			$feed
				->values($tmp_result)
				->save();
